...
export function FlowDescriptionPage({ language, objectifPrincipal, onBack, onCreate, onAnimationComplete }: FlowDescriptionPageProps) {
  ...
  const title = language === 'fr' ? 'Parfait, dis-nous en un peu plus' : language === 'en' ? 'Perfect, tell us a bit more' : 'Perfecto, cu├®ntanos un poco m├ís';
  const subtitle = language === 'fr' ? 'Sur ton objectif et ta situation actuelle...' : language === 'en' ? 'About your goal and current situation...' : 'Sobre tu objetivo y tu situaci├│n actual...';
  const placeholder = language === 'fr' ? 'Ex: Je veux am├®liorer ma confiance en moi car je me sens souvent anxieux en public...' : language === 'en' ? 'Ex: I want to improve my self-confidence because I often feel anxious in public...' : 'Ej: Quiero mejorar mi confianza porque a menudo me siento ansioso en p├║blico...';
  const createButton = language === 'fr' ? 'Cr├®er mon Flow' : language === 'en' ? 'Create my Flow' : 'Crear mi Flow';
  const backButton = language === 'fr' ? 'Retour' : language === 'en' ? 'Back' : 'Volver';
  const labelText = language === 'fr' ? 'D├®cris ton objectif et ta situation' : language === 'en' ? 'Describe your goal and situation' : 'Describe tu objetivo y tu situaci├│n';
  ...
  useEffect(() => {
    if (isGenerating) {
      // 1. Animation visuelle (boucle sur les ├®tapes)
