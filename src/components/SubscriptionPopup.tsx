...
export function SubscriptionPopup({ isOpen, onClose, onOpenAuthDialog }: SubscriptionPopupProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="relative w-full max-w-sm rounded-[1.5rem] shadow-2xl shadow-pink-200/50 overflow-hidden bg-white/95 backdrop-blur-xl border border-pink-100/50 animate-in zoom-in-95 duration-300">
        ...
        <Button
          ...
          size="icon"
          onClick={onClose}
          className="absolute top-1 right-1 z-10 rounded-full hover:bg-pink-100 w-7 h-7"
        >
        ...
        <div className="px-5 py-4 space-y-3 pt-6">
          ...
          <div className="flex justify-center">
            <div className="relative">
              ...
              />
              <Crown className="absolute -top-1 -right-1 w-6 h-6 text-pink-500 drop-shadow-xl animate-bounce" />
            </div>
          </div>
          ...
          <Button
            ...
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-pink-400 via-rose-400 to-orange-300 hover:from-pink-500 hover:via-rose-500 hover:to-orange-400 text-white font-bold py-4 text-sm shadow-xl shadow-pink-200/50 hover:shadow-pink-300/50 hover:scale-[1.02] transition-all rounded-xl"
          >
            {isLoading ? (
              'Traitement...'
            ) : (
